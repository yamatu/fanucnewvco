...
// search built from the first token of the administrator's message. Anything
// the guess missed was invisible to the model, so it answered from the wrong
// data or asked the administrator to paste records by hand.
//
...
const (
	...
	// It matches the batch tool and the bulk import so one pasted model list
	// can still be reviewed entry by entry.
	aiAgentMaxCreateProductProposals = aiAgentMaxBulkImportModels
	// aiAgentMaxBatchCreateItems bounds one create_products tool call.
...
// applyProductSearchFilter appends the shared product search condition to a
// query. The predicate mirrors the admin product list so tool results and the
// administrator's own search can never disagree about which rows match.
func applyProductSearchFilter(query *gorm.DB, search string) *gorm.DB {
	...
	like := "%" + search + "%"
	return query.Where("sku LIKE ? OR name LIKE ? OR description LIKE ? OR part_number LIKE ? OR model LIKE ?", like, like, like, like, like)
}
...
// The whole taxonomy is loaded in one query and walked in memory: it is small
// compared with the product catalogue, and building every path up front keeps
// the tool from issuing one query per id.
func aiAgentCategoryPathIndex(db *gorm.DB, ids []uint) (map[uint]string, error) {
	var all []models.Category
	if err := db.Model(&models.Category{}).Select("id", "name", "parent_id").Find(&all).Error; err != nil {
		return nil, err
	}
	...
	paths := make(map[uint]string, len(ids))
	for _, id := range ids {
		if id == 0 {
			continue
		}
		...
		paths[id] = aiAgentCategoryPath(id, byID)
	}
	return paths, nil
}
...
// describeAIAgentToolCall renders a short label for the admin UI trace, without
// echoing raw arguments back into the page.
