...
// category, one-click fix, resume, container restart - goes through
// dispatchQueuedAISEOJobs, so the ceiling holds however the job was created.
// When a job finishes, pauses or fails, its slot is released and the dispatcher
// is woken to hand it to the oldest queued job: first in, first out, so a queue
// of small jobs cannot be starved by one large job.
// ---------------------------------------------------------------------------
