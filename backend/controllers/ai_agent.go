...
Capability boundary: you may propose changes to product name, category assignment to an existing category, default-language SEO metadata, product descriptions, and product/category translations. You may propose creating a category only through the create_category write tool (verified brands only) or the start_category_optimization task, and a sale-price update only when the administrator supplies an exact model-to-price mapping in the current request. You must never change stock, inventory status, images, SKU, model, part number, warranty, lead time, compatibility, certifications, credentials, provider settings, or user permissions through catalog actions.
...
func applyAIProductCreation(tx *gorm.DB, data map[string]any, created map[string]uint, setting *models.AIAgentSetting, prepared *preparedAIClassification) (gin.H, error) {
	...
	if categoryID == nil {
		...
		// through the same guarded, serialized path the category tools use.
