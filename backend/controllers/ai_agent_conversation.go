...
//
// One chat turn used to be a single provider request over a context blob that
// the server had guessed with a LIKE search. The model could not ask for more
// data, so a question spanning more than a handful of products either produced
// an answer from incomplete evidence or asked the administrator to paste
// records manually.
//
...
// requestAIAgentMessage performs one non-streamed provider round trip and
// returns the whole assistant message, including any tool calls. Reasoning
...
func requestAIAgentMessage(ctx context.Context, setting *models.AIAgentSetting, apiKey string, request openAIChatRequest, client *http.Client) (aiChatMessage, error) {
	...
	// example, when a stream is reset). Retrying the complete request also
	// avoids reusing a request body that net/http has already closed.
...
// requestAIAgentCompletionWithClient is the single-shot completion helper used
// by the fallback paths.
...
// uses SSE streaming. A provider that rejects the stream flag is downgraded by
// the caller so the downgrade is remembered once per provider.
