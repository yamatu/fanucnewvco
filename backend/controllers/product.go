...
func findProductBySKUInternal(sku string) (models.Product, error) {
    ...
    add := func(s string) {
        ...
        if s == "" { return }
        if !candMap[s] { candMap[s] = true; candidates = append(candidates, s) }
    }
    add(normalized)
    if strings.HasPrefix(upper, "FANUC-") { add(normalized[6:]) }
    if strings.HasPrefix(upper, "FANUC ") { add(normalized[6:]) }
    add(upper)
...
func (pc *ProductController) LookupSEO(c *gin.Context) {
    ...
    if req.SourceBaseURL == "" { req.SourceBaseURL = "https://fanucworld.com" }
    ...
    if strings.ToLower(strings.TrimSpace(req.Provider)) == "ebay" {
        ...
        if eerr == nil && tmp != nil { extracted = tmp; candidate = src }
        if candidate != "" { attempted = append(attempted, candidate) }
    } else {
        ...
        for _, cand := range candidates {
            ...
            if strings.Contains(strings.ToLower(tmp.Title), "tie industrial - fanucworld.com") { continue }
            if strings.Contains(strings.ToLower(tmp.MetaDescription), "warning: target url returned error") { continue }
            extracted = tmp
            break
        }
    }
    ...
    if strings.Contains(s, "/product/") || strings.Contains(s, "/products/") {
        ...
        if u, err := neturl.Parse(s); err == nil {
            ...
            if len(segs) >= 2 {
                ...
                hostWWW := u.Host
                if !strings.HasPrefix(u.Host, "www.") { hostWWW = "www." + u.Host }
                // alt fanuc- prefix
                alt := last
                if strings.HasPrefix(last, "fanuc-") { alt = strings.TrimPrefix(last, "fanuc-") } else { alt = "fanuc-" + last }
                list = append(list,
        ...
        out := []string{}
        for _, it := range list { it = strings.TrimSpace(it); if it=="" {continue}; if !uniq[it] { uniq[it]=true; out=append(out,it) } }
        return out, out
    }
...
func (pc *ProductController) BulkAutoImportSEO(c *gin.Context) {
    ...
    if req.SourceBaseURL == "" { req.SourceBaseURL = "https://fanucworld.com/products/" }
    if req.Limit <= 0 || req.Limit > 500 { req.Limit = 50 }

    db := config.GetDB()
    type target struct{ ID uint; SKU string }
    targets := []target{}

    if len(req.ProductIDs) > 0 {
        var prods []models.Product
        if err := db.Where("id IN ?", req.ProductIDs).Find(&prods).Error; err == nil {
            for _, p := range prods { targets = append(targets, target{ ID:p.ID, SKU:p.SKU }) }
        }
    }
    if len(req.SKUs) > 0 {
        var prods []models.Product
    ...
    if len(targets) > req.Limit { targets = targets[:req.Limit] }
    if len(targets) == 0 {
    ...
    for _, t := range targets {
        ...
        if strings.ToLower(strings.TrimSpace(req.Provider)) == "ebay" {
            ...
            if err == nil && tmp != nil { extracted = tmp; candidate = src }
        } else {
            candidates, _ := buildCandidates(req.SourceBaseURL, product.SKU)
            for _, cand := range candidates {
                candidate = cand
        ...
        if req.Apply {
            product.MetaTitle = extracted.Title
            if extracted.MetaDescription != "" { product.MetaDescription = extracted.MetaDescription }
            if extracted.MetaKeywords != "" { product.MetaKeywords = extracted.MetaKeywords }
            if extracted.DescriptionHTML != "" {
                if strings.TrimSpace(product.Description) == "" {
                    product.Description = extracted.DescriptionHTML
                } else if !strings.Contains(product.Description, extracted.DescriptionHTML) {
            ...
            if req.AutoCategory && strings.TrimSpace(extracted.CategoryGuess) != "" {
                ...
                if cat.ID != 0 { product.CategoryID = cat.ID }
            }
            _ = db.Save(&product).Error
        }
        status := "fetched"
        if req.Apply { status = "applied" }
