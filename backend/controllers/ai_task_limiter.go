...
//    on four requests. It is taken per provider request, at the leaves that
//    issue one (doRequestAIAgentMessageStream, requestAIAgentMessage and
//    requestAIAgentCompletion); those leaves never call each other, so one
//    request can never take a slot twice, which would deadlock the gate.
//
...
// slot for a job's whole lifetime let one long job - a category task walking
// through tens of thousands of items for days - occupy a permanent share of the
// ceiling and make every other task wait behind it. This gate only bounds how
// many requests the running tasks may have in flight together.
//
