...
type AIAgentSetting struct {
	...
	// product SEO jobs and category optimization jobs each hold one task slot for
	// their whole lifetime, and a task beyond the ceiling stays queued until one
	// of them finishes. SEOJobConcurrency answers "how many provider requests
	// inside one task"; this answers "how many tasks at all". Interactive
	// assistant turns take a request slot but never a task slot, so chatting
	// never queues behind the background jobs.
