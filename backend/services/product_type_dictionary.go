...
// CanonicalProductType maps a free-form type name onto the stable vocabulary
// entry when one exists; otherwise it only normalizes whitespace.
...
// published as a category. Kept in one place so the assistant, the classifier
// and the category task all agree on what "not a real product type" means.
