...
SELECT
  ...
  1 AS is_active,
  0 AS is_featured,
  CONCAT('SICK ', seed.sku, ' ', seed.product_type, ' | Vcocnc') AS meta_title,
  CONCAT(
    'SICK ', seed.sku, ' ', seed.product_type,
    ' for industrial automation repair and replacement. Compatibility support, 12-month warranty, and worldwide shipping from Vcocnc.'
  ) AS meta_description,
  CONCAT(
    seed.sku, ', SICK ', seed.series_name, ', ', seed.product_type,
    ', SICK AG, industrial sensors, machine safety, automation parts, Vcocnc'
  ) AS meta_keywords,
