  ...
  const handleUpload = async (autoApply = false) => {
    ...
    try {
      ...
      if (autoApply) {
        ...
        const next = await BackupService.applyProductCatalogImport(result.job.id, { conflict_policy: 'upsert', create_categories: true, overwrite_local_files: false, brand_map: directBrandMap, text_replacements: [] });
        setJob(next);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 space-y-5">
      ...
      <div className="border-t border-gray-200 pt-4 space-y-3">
        ...
        <button type="button" onClick={() => handleUpload(true)} disabled={!file || busy !== null || Boolean(active)} className="inline-flex items-center rounded-md border border-emerald-600 px-4 py-2 text-sm font-medium text-emerald-700 hover:bg-emerald-50 disabled:opacity-60">{zh ? '上传并直接同步' : 'Upload & Sync Directly'}</button>
      </div>
