          ...
          <form onSubmit={save} className="space-y-6">
            ...
            <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-5 rounded-xl border border-violet-100 bg-violet-50/60 p-4">
                ...
                <p className="mt-1 text-sm text-violet-900/80">{zh ? '产品页的“自动候选优化”会优先选择启用、未 AI 优化且内容较薄弱的商品，也支持单独重试此前 AI 优化失败的商品。每次最多 30000 个；可以暂停后保留队列并继续执行，容器重启后未完成任务也会续跑。' : 'Automatic candidate optimization prioritizes active, not-yet-AI-optimized products with thinner content, and a separate retry mode can target products whose previous AI attempt failed. Each batch is capped at 30000; you can pause while keeping the queue and resume later, and unfinished work resumes after a container restart.'}</p>
