...
后台右下角的「AI 优化」按钮提供受控的目录优化对话：它可以分析商品归类、提出缺失分类、修正错误分类，并生成商品或分类的多语言 SEO 内容。
...
2. 输入 SKU、商品名称或分类目标。例如：
   ...
   - `检查这类 I/O 模块是否有合适分类；没有则提出创建分类建议`
3. AI 会列出可展开的建议卡，卡内可查看要创建的分类、目标分类、meta title、meta description、keywords 和语言版本内容。
4. 逐项点 **应用**，或在同一轮建议中点 **确认并应用本轮全部建议**。

当同一轮需要“新建分类 → 将商品移入该分类”时，应使用“确认并应用本轮全部建议”，后端会用一个数据库事务按顺序执行。
...
4. AI 会收到现有分类目录：有正确分类时必须选择该分类；仅在没有任何现有分类能准确描述产品时，才会创建一个非重复的新分类并将产品移入。名称、描述和分类只能依据商品已有的 SKU、名称、品牌、型号、料号、分类与原始资料生成；不会编造规格、兼容性、认证、库存、质保或性能，也不会修改价格、库存、商品图片或其他字段；
...
- 仅允许四种动作：创建分类、修改商品分类/默认语言 SEO、写入商品翻译、写入分类翻译。
- 不允许 AI 变更价格、库存、订单、用户、权限或任意数据库列。
- 分类创建、同一轮分类移动和翻译写入在数据库事务中完成；失败时整轮回滚。
